...
function createSphere(gl, radius)
{
  ...
  var circle = {x: 0, y: 0, r: .5};
  var attribsPerVert = 2;
  var numFans = 360;
  var degreePerFan = (2 * Math.PI) / numFans;
  var verts = [circle.x, circle.y];

  for(var i = 0; i <= numFans; i++) {
    var index = attribsPerVert * i + 2;
    var angle = degreePerFan * (i+1);
    if(angle > 0.30 && angle <= 5.7) {
      verts[index] = circle.x + Math.cos(angle) * circle.r;
      verts[index + 1] = circle.y + Math.sin(angle) * circle.r;
    }
  }

  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);

  var sphere = {buffer:vertexBuffer, vertSize:2, nVerts:verts.length/attribsPerVert, primtype:gl.TRIANGLE_FAN};
  return sphere;
}
