 ...
 *
 * 	https://github.com/speculees/PhysicsFirstPersonControls
 *
 *	Pequeñas modificaciones por Jabobo y Alejandro
 */
...
THREE.PhysicsFirstPersonControls = function (object) {
	...
	this.raycasterFront = new THREE.Raycaster(this.camera.getWorldPosition(), this.camera.getWorldDirection());

	this.raycasterBottom = new THREE.Raycaster(new THREE.Vector3(), new THREE.Vector3(0, -1, 0), 0, 10);
	...
	this.stepWalkDistance = 24;
	this.stepRunDistance = 32;
	...
	this.maxWalkVelocity = 20; //Walking speed
	this.maxRunVelocity = 35; //Running speed
	...
	function move(delta) {
		if (!isEmpty(keys)) {
			...
			var timeThisRound = window.performance.now(); //get current time
